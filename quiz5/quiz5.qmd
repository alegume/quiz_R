...
Considerando um nível de significância de 5%, não rejeita-se a hipótese de que os dados seguem a distrição normal, pois os p-valores são maiores do que 0,05 para todos os testes de normalidades.
...
Todos os testes apresentaram p-valores que não justificam rejeitar as respectivas hipóteses nulas (que os dados seguem as distribuições Gamma, Normal e Weibull, respectivamente), com um nível de significância de 5%. Entretanto, a escolha da função de distribuição que melhor ajusta os dados é a Weibull, pois além de apresentar p-valor = 0.9624, é a que obteve melhor Critério de Informacao de Akaike.

#### c) Plotar a função e o histograma para distribuição escolhida.

```{r warning=FALSE}
w_2_1 <- rweibull(1500, shape = 6.513198, scale = 1.918411)
...
Considerando um nível de significância de 5%, não rejeita-se a hipótese de que os dados amostrais de inflação seguem a distrição normal, considerando que ambos p-valores são maiores do que 0,05.
...
Portanto, o intervalo de confiaça de 99% para média da amostra da inflação é \[3.426025 17.432443\].
...
lillie.test(inflacao)
```
Entretanto, pelo teste de Lilliefors (p-value = 0.01631), rejeita-se a hipótese de que os dados da inflação para os anos de 1999 até 2022 seguem a distrição normal, com 5% de significância.
...
id_dist(dados);
```
A distribuição que apresenta melhor Critério de Informacao de Akaike é a **Gamma**. Portanto, realiza-se o teste de Kolmogorov-Smirnov e não se rejeita a hipótese de que os dados seguem a distrbuição **Gamma**, com um nível de significância de 5%.
```{r}
...
```
A distribuição que apresenta melhor Critério de Informacao de Akaike é a **Gamma**. Portanto, realiza-se o teste de Kolmogorov-Smirnov e não se rejeita a hipótese de que estes dados também seguem a distrbuição **Gamma**, com um nível de significância de 5%.
```{r}
fitdist(dados, "gamma", method="mle")
...
A distribuição que apresenta melhor Critério de Informacao de Akaike é a **Lognormal**. Portanto, realiza-se o teste de Kolmogorov-Smirnov e não se rejeita a hipótese de que os dados seguem a distrbuição **Lognormal**, com um nível de significância de 5%.
```{r}
...
id_dist(dados);
```
A distribuição que apresenta melhor Critério de Informacao de Akaike é a **Weibull**. Portanto, realiza-se o teste de Kolmogorov-Smirnov e não se rejeita a hipótese de que os dados seguem a distrbuição **Weibull**, com um nível de significância de 5%.
```{r}
...
id_dist(dados);
```
A distribuição que apresenta melhor Critério de Informacao de Akaike é a **Uniforme**. Portanto, realiza-se o teste de Kolmogorov-Smirnov e não se rejeita a hipótese de que os dados seguem a distrbuição **Uniforme**, com um nível de significância de 5%.
```{r}
